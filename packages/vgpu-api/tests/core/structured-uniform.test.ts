...
import { StructuredUniform, type WgslUniformType } from "../../src/core.ts";
...
test("derives the expected WGSL uniform layout for a mixed schema", () => {
  const device = makeDevice();
  const schema = { a: "f32", b: "vec2f", c: "vec3f", d: "mat3x3f", e: "mat4x4f" } as const;

  const uniform = new StructuredUniform(device, { schema });

  expect(uniform.byteSize).toBe(144);
  expect(uniform.offsets).toEqual({ a: 0, b: 8, c: 16, d: 32, e: 80 });
  expect(uniform.layout.byteSize).toBe(144);
  expect(uniform.layout.offsets).toEqual({ a: 0, b: 8, c: 16, d: 32, e: 80 });
  device.destroy();
});
