...
test("named geometry attributes resolve to reflected vertex locations at gpu.draw construction", async () => {
  ...
  try {
    const geometry = gpu.geometry({ buffers: [{ data: new Float32Array(15), attributes: {
      uv: "float32x2",
      position: "float32x3",
    } }] });
    const shader = `
    ...
    gpu.draw({ shader, geometry }).draw(target);
...
test("explicit locations win over attribute names", async () => {
  const gpu = await init();
  try {
    const geometry = gpu.geometry({ buffers: [{ data: new Float32Array(2), attributes: { label_only: { format: "float32x2", location: 3 } } }] });
...
test("draw construction reports unmatched, ambiguous, missing, and format mismatch geometry inputs", async () => {
  const gpu = await init();
  try {
    const named = (attributes: Record<string, GPUVertexFormat | { format: GPUVertexFormat; location?: number }>) => gpu.geometry({ buffers: [{ data: new Float32Array(4), stride: 16, attributes }] });
    const unmatched = errorOf(() => gpu.draw({ shader: `@vertex fn vs_main(@location(0) position: vec2f) -> @builtin(position) vec4f { return vec4f(position, 0.0, 1.0); } ${FS}`, geometry: named({ color: "float32x4" }) }));
    const ambiguous = errorOf(() => gpu.draw({ shader: `struct In { @location(0) value: vec2f } @vertex fn vs_main(input: In, @location(1) value: vec2f) -> @builtin(position) vec4f { return vec4f(input.value + value, 0.0, 1.0); } ${FS}`, geometry: named({ value: "float32x2" }) }));
    const missing = errorOf(() => gpu.draw({ shader: `@vertex fn vs_main(@location(0) position: vec2f, @location(1) uv: vec2f) -> @builtin(position) vec4f { return vec4f(position + uv, 0.0, 1.0); } ${FS}`, geometry: named({ position: "float32x2" }) }));
    const mismatch = errorOf(() => gpu.draw({ shader: `@vertex fn vs_main(@location(0) position: vec2u) -> @builtin(position) vec4f { return vec4f(position); } ${FS}`, geometry: named({ position: "float32x2" }) }));
...
test("structural GeometryLike escape hatches skip name and format checks", async () => {
  const gpu = await init();
  try {
    expect(() => gpu.draw({ shader: `@vertex fn vs_main(@location(7) anything: vec4u) -> @builtin(position) vec4f { return vec4f(anything); } ${FS}`, geometry: {
      vertexCount: 1,
