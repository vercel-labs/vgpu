...
export async function createGpu(entry: "browser" | "node" | "mock", opts: InitOptions = {}, _unused: InitOptions = {}, adapterFactory?: AdapterFactory): Promise<Gpu> {
  ...
  try {
    ...
    // An already-destroyed external device must not leave a half-built Gpu behind.
    device.dispose();
    throw error;
  }
}
...
class RingGpu implements Gpu {
  ...
  constructor(readonly device: Device) {
    ...
    const runner = new FrameRunner(() => { this.#assertUsable(); return new Frame(device, undefined, (error) => this.#reportError(error), (promise) => this.#trackDelivery(promise), () => this.#clearColorValue); }, () => this.#advanceFrameState(), (handle) => this.#registerLoop(handle));
  ...
  async settled(): Promise<void> {
    ...
    // Re-checked after the await: a retained external device can be destroyed while work drains.
    this.#assertUsable();
  }
