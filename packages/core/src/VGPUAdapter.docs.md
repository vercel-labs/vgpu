...
| opts.requiredLimits | `Record<string, number>` | ✖ | `undefined` | Forwarded to native `adapter.requestDevice({ requiredLimits })` by browser/node implementations. The mock adapter ignores it. |
