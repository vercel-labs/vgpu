...
test("cancelling at the actual empty prepared response preserves the original empty root without sending commit", async () => {
  const input = await projectFixture();
  const controller = new AbortController();
  const reason = new Error("cancelled at actual empty prepared response");
  ...
  try {
    ...
    expect(failure).toMatchObject({ recoveryPaths: [stage, journal] });
    for (const root of [
      await lstat(input.outputPath, { bigint: true }),
