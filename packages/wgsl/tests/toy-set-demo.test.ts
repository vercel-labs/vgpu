...
test("toy effect(gpu).set() prototype writes reflected uniforms in-place with stable bind groups on mock", async () => {
...
test.skipIf(process.env.VGPU_DOCKER_TEST !== "1")("toy effect(gpu).set() prototype renders offscreen using reflected layout on Dawn", async () => {
