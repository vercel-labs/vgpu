...
describe("healManifold", () => {
  test("removes non-manifold residue and restores closed manifold topology", () => {
    ...
    expect(result.report.nonManifoldEdgesFixed).toBe(1);
  });

  test("preserves sharp flags on retained edges", () => {
    const em = nonManifoldTetra(); unwrapKernel(em.gpu.halfEdgeKernel).isSharp[edgeBetween(em, 2, 3)] = 1;
    const result = healManifold(em);
    expect(unwrapKernel(result.mesh.gpu.halfEdgeKernel).isSharp[edgeBetween(result.mesh, 2, 3)]).toBe(1);
  });
