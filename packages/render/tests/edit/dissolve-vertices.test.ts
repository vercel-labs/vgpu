...
describe("dissolveVertices", () => {
  ...
  test("returns surrounding face descendants", () => {
    const em = octahedron(), result = dissolveVertices(em, em.vertices.byIndex([0]));
    expect(result.descendants.surroundingFaces.domain).toBe("face");
    expect(result.descendants.surroundingFaces.indices).toEqual([4, 5]);
  });
  ...
  test("propagates survivor face smoothing with OR and preserves remaining sharp edges", () => {
    ...
    const result = dissolveVertices(em, em.vertices.byIndex([0])), out = result.mesh.gpu.halfEdgeKernel;
    expect(result.descendants.surroundingFaces.indices.every((f) => out.useSmooth[f] === 1)).toBe(true);
  ...
  test("warns and skips a T-junction boundary vertex", () => {
    ...
    expect(result.descendants.surroundingFaces.count).toBe(0);
  });
});
