...
describe("gridFill", () => {
  ...
  test("returns all inserted grid-fill triangles", () => {
    ...
    expect(result.descendants.newFaces.indices).toEqual([em.faceCount, em.faceCount + 1, em.faceCount + 2, em.faceCount + 3]);
  });
  ...
  test("preserves boundary sharp edges and makes new faces smooth", () => {
    ...
    const result = gridFill(em, boundary), k = result.mesh.gpu.halfEdgeKernel;
