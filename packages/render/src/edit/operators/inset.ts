...
export interface InsetResult { readonly mesh: EditableMesh; readonly descendants: { readonly insetFaces: ElementSelection; readonly boundaryFaces: ElementSelection; readonly rimEdges: ElementSelection }; readonly warnings?: readonly MeshEditWarning[] }
...
export function inset(em: EditableMesh, faces: ElementSelection, opts: InsetOptions): InsetResult {
  ...
  const inner: number[] = [], boundary: number[] = [], warnings: MeshEditWarning[] = [], source = em.gpu.halfEdgeKernel;
  ...
  const out = { mesh, descendants: { insetFaces: selection("face", inner), boundaryFaces: selection("face", boundary), rimEdges } };
...
function edgeSelectionOf(mesh: EditableMesh, faces: readonly number[]): ElementSelection { const s = new Set(faces), out: number[] = [], k = mesh.gpu.halfEdgeKernel; for (let e = 0; e < k.edgeCount; e++) if (s.has(k.edgeFaceA[e]) || s.has(k.edgeFaceB[e])) out.push(e); return selection("edge", out); }
