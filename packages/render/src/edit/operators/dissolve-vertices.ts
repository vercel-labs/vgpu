...
export interface DissolveVerticesResult { readonly mesh: EditableMesh; readonly descendants: { readonly surroundingFaces: ElementSelection }; readonly warnings?: readonly MeshEditWarning[] }
...
export function dissolveVertices(em: EditableMesh, vertices: ElementSelection, _opts: DissolveVerticesOptions = {}): DissolveVerticesResult {
  ...
  const k = em.gpu.halfEdgeKernel, faces = new Set<number>(), warnings: MeshEditWarning[] = [];
  ...
  if (!faces.size) {
    ...
    const out = { mesh, descendants: { surroundingFaces: selection("face", []) } };
    return warnings.length ? { ...out, warnings } : out;
  }
  const dissolved = dissolveFaces(em, selection("face", [...faces]));
  const out = { mesh: dissolved.mesh, descendants: { surroundingFaces: dissolved.descendants.resultFace } };
