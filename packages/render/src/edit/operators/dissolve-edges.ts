...
export interface DissolveEdgesResult { readonly mesh: EditableMesh; readonly mergedFaces: ElementSelection; readonly warnings?: readonly MeshEditWarning[] }
...
export function dissolveEdges(em: EditableMesh, edges: ElementSelection, _opts: DissolveEdgesOptions = {}): DissolveEdgesResult {
  ...
  const k = unwrapKernel(em.gpu.halfEdgeKernel), jobs: Job[] = [], drop = new Set<number>(), warnings: MeshEditWarning[] = [];
...
function addMergedQuad(em: EditableMesh, m: MeshParts, job: Job): void {
  ...
  if (k.isSharp[job.edge]) for (const [x, y] of [[c, a], [a, d], [d, b], [b, c]] as const) m.sharp.add(edgePosKey(em, x, y));
}
...
function edgePosKey(em: EditableMesh, a: number, b: number): string { const av = q(p(em, a)), bv = q(p(em, b)); return av < bv ? `${av}|${bv}` : `${bv}|${av}`; }
