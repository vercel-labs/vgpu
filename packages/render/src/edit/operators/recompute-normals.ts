...
export function recomputeNormals(em: EditableMeshValue, opts: RecomputeNormalsOptions = {}): EditableMeshValue {
  if (em.faceCount === 0) return em;
...
function smoothComponent(em: EditableMeshValue, seed: number, visited: Uint8Array): number[] {
  const k = unwrapKernel(em.gpu.halfEdgeKernel), out: number[] = [], stack = [seed]; visited[seed] = 1;
  while (stack.length) {
