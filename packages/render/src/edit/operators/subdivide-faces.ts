...
export interface SubdivideFacesResult { readonly mesh: EditableMesh; readonly newFaces: ElementSelection; readonly newEdges: ElementSelection }
...
export function subdivideFaces(em: EditableMesh, faces: ElementSelection, opts: SubdivideFacesOptions = {}): SubdivideFacesResult {
  ...
  for (let i = 1; i < Math.max(1, Math.floor(opts.cuts ?? 1)); i++) result = once(result.mesh, result.newFaces);
  return result;
}

function once(em: EditableMesh, faces: ElementSelection): SubdivideFacesResult {
  const selected = new Set(faces.indices), parts: MeshParts = { positions: [], faces: [], useSmooth: [], sharp: new Set() }, k = unwrapKernel(em.gpu.halfEdgeKernel);
  ...
  return { mesh, newFaces: descFaces, newEdges: edgesByKeys(mesh, newEdgeKeys) };
}

function copySharp(em: EditableMesh, parts: MeshParts, f: number): void { const k = unwrapKernel(em.gpu.halfEdgeKernel); for (const e of k.faceEdges.slice(f * 3, f * 3 + 3)) if (k.isSharp[e]) parts.sharp.add(key(p(em, k.edgeVertexA[e]), p(em, k.edgeVertexB[e]))); }
