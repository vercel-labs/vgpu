...
test.runIf(process.platform==="win32")("retries Windows parent-lock contention for simultaneous pulls to one destination",async()=>{const root=await mkdtemp(join(tmpdir(),"pull-windows-same-parent-"));cleanup.push(()=>rm(root,{recursive:true,force:true}));const out=join(root,"out"),bytes=Buffer.from("ok"),manifest:any={files:[{path:"only.txt",size:2,sha256:"x"}]},client:any={getFile:async()=>bytes};const results=await Promise.allSettled(Array.from({length:3},()=>pullExample(client,manifest,out)));expect(results.filter(x=>x.status==="fulfilled")).toHaveLength(1);const rejections=results.filter((x):x is PromiseRejectedResult=>x.status==="rejected");expect(rejections).toHaveLength(2);expect(rejections.map(x=>x.reason.code)).toEqual(["VGPU-EXAMPLES-DESTINATION-EXISTS","VGPU-EXAMPLES-DESTINATION-EXISTS"]);expect(await readFile(join(out,"only.txt"),"utf8")).toBe("ok")});

test.runIf(process.platform==="win32")("retries Windows parent-lock contention for independent destinations",async()=>{const root=await mkdtemp(join(tmpdir(),"pull-windows-shared-parent-"));cleanup.push(()=>rm(root,{recursive:true,force:true}));const bytes=Buffer.from("ok"),manifest:any={files:[{path:"only.txt",size:2,sha256:"x"}]},client:any={getFile:async()=>bytes},outputs=Array.from({length:8},(_,i)=>join(root,`out-${i}`)),results=await Promise.allSettled(outputs.map(out=>pullExample(client,manifest,out)));expect(results).toEqual(outputs.map(()=>expect.objectContaining({status:"fulfilled"})));expect(await Promise.all(outputs.map(out=>readFile(join(out,"only.txt"),"utf8")))).toEqual(outputs.map(()=>"ok"))});

test.runIf(process.platform==="win32")("retries a disappearing destination lock beneath the Windows current directory",async()=>{const root=await mkdtemp(join(tmpdir(),"pull-windows-cwd-race-")),previous=process.cwd(),bytes=Buffer.from("ok"),manifest:any={files:[{path:"only.txt",size:2,sha256:"x"}]},client:any={getFile:async()=>bytes};cleanup.push(()=>rm(root,{recursive:true,force:true}));try{process.chdir(root);const results=await Promise.allSettled(Array.from({length:3},()=>pullExample(client,manifest,"out")));expect(results.filter(x=>x.status==="fulfilled")).toHaveLength(1);const rejections=results.filter((x):x is PromiseRejectedResult=>x.status==="rejected");expect(rejections).toHaveLength(2);expect(rejections.map(x=>x.reason.code)).toEqual(["VGPU-EXAMPLES-DESTINATION-EXISTS","VGPU-EXAMPLES-DESTINATION-EXISTS"]);expect(await readFile(join(root,"out","only.txt"),"utf8")).toBe("ok")}finally{process.chdir(previous)}});
