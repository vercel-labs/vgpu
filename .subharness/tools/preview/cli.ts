...
//     [--width 1280] [--height 720] [--settle 2500] [--path /examples/<slug>] [--wait-for canvas]
//     [--reduced-motion]
//
...
if (!slug || slug.startsWith("--")) {
  console.error("usage: node .subharness/tools/preview/cli.ts <slug> [--steps <json|@file>] [--width n] [--height n] [--settle ms] [--path p] [--wait-for sel] [--reduced-motion]");
  process.exit(2);
}
const flags = new Map<string, string>();
for (let index = 0; index < rest.length; index++) {
  const name = rest[index].replace(/^--/, "");
  if (name === "reduced-motion") flags.set(name, "true");
