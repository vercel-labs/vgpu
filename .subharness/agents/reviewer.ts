...
5. Release hygiene: changeset present, meaningfully named, and accurate when published behavior changed (including bundled docs); Summary/Migration describe the final behavior; no package version bumps; \`pnpm migrations:check\` passes; bundle budgets hold.
6. Workflow: for maintainer-adoption work, the diff is an independent implementation — flag code copied or cherry-picked from the external PR. Canvas formats are not hardcoded; CI visual reference snapshots were not overwritten locally; shaders are compat-safe.
