...
3. Register the slug and get something on screen early, then iterate with capture_preview: script the brief's interactions (bursts for motion, gui to compare parameters, perf at dpr 2), look at every frame critically, fix, repeat, including 832×468 and a touch phone. As soon as the pipeline renders, write a first render-thumbnail.ts and start the Mesa check in the background (\`node .subharness/tools/thumbs/mesa.ts <slug> --update\`): CI's compatibility-mode renderer is the only validator for some WGSL (verify_example lints the known cases), and finding a compat error late costs a full re-verification.
4. Write the tests. Update design.md to match what you built.
5. Run subagent:example-reviewer once with the brief path, design.md, the base ref (\`git merge-base HEAD origin/canary\`), and the checks you ran; save its reply to .context/work/<topic>/<slug>/review.md. Fix blocker/major findings (and cheap minors); record anything you dispute.
