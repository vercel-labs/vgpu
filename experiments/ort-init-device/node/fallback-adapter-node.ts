...
import { createNodeDevice } from "../../../packages/adapter-node/dist/index.js";
...
const artifact = resolve(process.env.ORT_EVIDENCE_DIR ?? resolve(root, "artifacts"), "node-fallback.json");
...
let providerDevice: Awaited<ReturnType<typeof createNodeDevice>> | undefined;
...
try {
  ...
  // Explicit host fallback: adapter-node resolves its portable Dawn and lavapipe artifacts.
  providerDevice = await createNodeDevice(); createCount++;
  const providerRaw = providerDevice.gpu;
  installGlobal("self", globalThis);
  assertions.singleDawnSingleton = createCount === 1;
  lifecycle.push("single-adapter-node-device-created");

  const adapter = Object.create((globalThis as any).GPUAdapter.prototype) as GPUAdapter;
  Object.defineProperties(adapter, {
    info: { configurable: true, value: providerDevice.adapterInfo ?? undefined },
    features: { configurable: true, value: new Set<GPUFeatureName>() },
    limits: { configurable: true, value: {} },
    isFallbackAdapter: { configurable: true, value: true },
    requestDevice: { configurable: true, value: async () => providerRaw },
  });
  const fallbackGpu = { requestAdapter: async () => adapter } as unknown as GPU;
  installGlobal("navigator", Object.assign(globalThis.navigator ?? {}, { gpu: fallbackGpu }));
  ...
  try { providerDevice?.dispose(); } catch (error) { errors.push(errorText(error)); }
...
const evidence: Evidence = { platform: "node", status: errors.length === 0 && pass(assertions) ? "PASS" : "FAIL", matrix: { node: process.version, webgpu: "adapter-node portable Dawn fallback", ort: "1.27.0", vgpu: "0.1.6", renderer: "software-renderer@0.1.6 (host fallback)" }, assertions, snapshot, reference, lifecycle, errors };
