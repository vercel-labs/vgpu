...
const { reflectSource } = await import(
  pathToFileURL(join(repoRoot, "packages/wgsl/dist/runtime/reflect-source.js"))
    .href
);

const canaries = [
  {
    name: "alignment-and-io",
    contract: "valid",
    tint: "success",
    naga: "success",
  },
  {
    name: "uniform-standard-layout",
    contract: "valid",
    tint: "success",
    naga: "failure",
    reason: "uniform_buffer_standard_layout is unsupported by Naga 30.0.1",
  },
  {
    name: "binding-slots",
    contract: "valid",
...
const resolverNegatives = new Map([
  [
    "packages/vgpu-api/tests/fixtures/bool-uniform.wgsl",
    "VGPU-WGSL-REFLECT-BOOL-HOST-SHAREABLE",
  ],
  [
    "packages/vgpu-api/tests/fixtures/module-binding-entry.wgsl",
    "VGPU-RESOLVE-MODULE-BINDING",
  ],
]);
const translatorNegative =
  "packages/vgpu-api/tests/fixtures/reserved-word.wgsl";
...
const overrideSelections = new Map([
  [
    "canaries/typed-overrides.wgsl",
    { GAIN: "2.0", WG_X: "4u", ENABLED: "true" },
  ],
...
const matchCount = (source, expression) =>
...
function bakeOverrides(source, selected, label) {
  ...
  const output = source.replace(
    ...
    (_declaration, name, type, defaultValue) => {
      ...
      if (!value)
        throw new Error(
          `${label}: override ${name} has no selected or default value`
        );
      overrides.push({
        name,
        type: type?.trim() ?? null,
...
function fallbackReflection(source, error) {
  return {
    ...
    entryPoints: [
      ...source.matchAll(
        /@(vertex|fragment|compute)\b[\s\S]*?\bfn\s+([A-Za-z_][A-Za-z0-9_]*)\s*\(/g
      ),
    ].map((match) => ({
      stage: match[1],
      name: match[2],
...
async function prepareInput({
  ...
  if (baked.overrides.length)
    bakedOverrides.push({ name, overrides: baked.overrides });
...
if (mode === "full") {
  ...
  const paths = execFileSync(
    ...
    [
      ...
      `!${fixtureRelative}/**`,
      "-g",
      "!experiments/native-metal-spikes/**",
    ],
  ...
  const sum = (select) =>
    records.reduce((total, record) => total + select(record), 0);
  ...
  inventory = {
    ...
    filesWithEntrypoints: records.filter((record) => record.entryPoints > 0)
      .length,
  ...
  const { resolveShader } = await import(
    pathToFileURL(
      join(repoRoot, "packages/wgsl/dist/runtime/resolve-shader.js")
    ).href
  );
  const failures = [];
  let deterministic = 0;
  for (const [index, path] of paths.entries()) {
    try {
      const first = await resolveShader({
        entry: join(repoRoot, path),
        validate: false,
        minify: false,
      });
      const second = await resolveShader({
        entry: join(repoRoot, path),
        validate: false,
        minify: false,
      });
      if (first.wgsl !== second.wgsl)
        throw new Error(`${path}: resolver output is not deterministic`);
      deterministic++;
      ...
      await prepareInput({
        ...
        naga:
          contract === "invalid" || nagaExtensionFailures.has(path)
            ? "failure"
            : "success",
        reason: nagaExtensionFailures.has(path)
          ? "unrestricted_pointer_parameters is unsupported by Naga 30.0.1"
          : undefined,
      });
    } catch (error) {
    ...
    if ((index + 1) % 50 === 0)
      console.error(`prepared ${index + 1}/${paths.length}`);
  }
  const unexpected = failures.filter((failure) => !failure.expected);
  const missingExpected = [...resolverNegatives.keys()].filter(
    (path) => !failures.some((failure) => failure.path === path)
  );
  if (unexpected.length || missingExpected.length) {
    throw new Error(
      `resolver outcome drift: ${JSON.stringify({
        unexpected,
        missingExpected,
      })}`
    );
