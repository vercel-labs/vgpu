...
linking. Its fixed direct profile also derives a nominal runtime layout, prepares buffers, one
sampled texture, and one filtering sampler, and passes exact readback. Exact-static override
extraction, assembly, translation, offline linking, and one live render observation also pass.
Broader resource runtime coverage, the authenticated repository corpus, and production artifact
packaging remain. Resolver, inventory, compiler-protocol, direct-build, offline Metal, and corpus
baselines must stay green while those pieces are connected. Intel, AMD, and cross-machine hardware
evidence also remain outside this run.
