...
extraction/assembly, broader resource runtime coverage, the authenticated repository corpus, and
production artifact packaging remain. Resolver, inventory, compiler-protocol, direct-build, offline
Metal, and corpus baselines must stay green while those pieces are connected. Intel, AMD, and
cross-machine hardware evidence also remain outside this run.
