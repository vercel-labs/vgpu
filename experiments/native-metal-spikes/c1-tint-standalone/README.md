...
are closed for the pinned profile. The connected semantic bridge now carries authenticated
fixed-size singular resources through nominal slot allocation, exact per-entry projection,
translation, and offline linking. The remaining gates are:

- extend extraction and assembly with exact-static overrides;
- extend the connected bridge to broader resource shapes and validate the complete corpus through
  offline `metal` and `metallib` at MSL 2.4;
