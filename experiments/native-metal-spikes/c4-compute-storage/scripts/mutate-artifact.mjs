...
switch (mutation) {
  case "abi":
    ...
    break;
  case "model":
    artifact.programs[0].runtimeManifest.storageBufferSizeModel =
  ...
  case "unknown":
    ...
    break;
  case "sampling":
    artifact.programs[1].runtimeManifest.samplingPairs = [{}];
    break;
  case "program-order":
...
function usage() {
  fail(
    "usage: mutate-artifact.mjs --artifact <artifact.json> --generated <generated.swift> --mutation <abi|model|unknown|sampling|program-order>"
  );
}

function fail(message) {
  process.stderr.write(`c4-compute-storage mutation helper: ${message}\n`);
  process.exit(1);
}
