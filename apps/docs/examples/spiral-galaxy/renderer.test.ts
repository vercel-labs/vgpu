...
function setup(options: { failCompile?: boolean; reducedMotion?: boolean } = {}) {
  ...
  const computes: Array<{ set: ReturnType<typeof vi.fn>; dispatch: ReturnType<typeof vi.fn>; compile: ReturnType<typeof vi.fn> }> = [];
  const surface = { size: [320, 160] as const, format: 'bgra8unorm', dispose: vi.fn() };
  ...
  const stop = vi.fn();
  const computeDispatch = vi.fn();
  const computePass = vi.fn((body: (pass: { dispatch: typeof computeDispatch }) => void) => {
    body({ dispatch: computeDispatch });
  });
  type TestFrame = { pass: ReturnType<typeof vi.fn>; computePass: typeof computePass };
  let liveFrame: ((frame: TestFrame) => void) | undefined;
  const frame = vi.fn((callback: (frame: TestFrame) => void) => {
    callback({ pass: vi.fn(), computePass });
  });
  const gpu = {
    ...
    fns: {
      ...
      draw: vi.fn(effect),
      compute: vi.fn(() => {
        const value = { set: vi.fn(), dispatch: vi.fn(), compile: vi.fn(async () => { await compile(); }) };
        computes.push(value);
        return value;
      }),
      sampler: vi.fn(() => ({})),
...
test('initializes the field, bakes the dirt map and drives the simulation every frame', async () => {
  ...
  expect(simulate.compile).toHaveBeenCalledOnce();
  expect(simulate.compile.mock.invocationCallOrder[0]).toBeLessThan(env.gpu.fns.frameLoop.mock.invocationCallOrder[0]!);
  expect(simulate.dispatch).not.toHaveBeenCalled();
  expect(env.computeDispatch).toHaveBeenCalledWith(simulate, Math.ceil(field.count / 64));
  expect(env.computeDispatch.mock.invocationCallOrder[0]).toBeLessThan(pass.mock.invocationCallOrder[0]!);
...
test('thumbnail renders deterministic frames and destroys its resources', async () => {
  ...
  expect(env.computes[0]!.dispatch).not.toHaveBeenCalled();
  expect(env.computeDispatch).toHaveBeenCalledTimes(3);
  expect(env.computes[0]!.set.mock.calls.at(-1)?.[0].params.intro).toBe(1);
...
test('unchanged physical size and DPR retain targets and hover motion', async () => {
  const env = setup();
  const renderer = createRenderer({ canvas: env.canvas });
  await renderer.ready;
  const flushResize = () => {
    const callbacks = [...env.frames.values()];
    env.frames.clear();
    callbacks.forEach((callback) => callback(16));
  };
  flushResize(); // Initial measurement matches the surface.
  env.runFrame();
  renderer.resize({ width: 200.1, height: 100.1, dpr: 1.6 });
  flushResize(); // Subpixel layout change rounds to the same backing size.
  env.runFrame();
  expect(env.gpu.fns.target).toHaveBeenCalledTimes(6);
  expect(env.storageObjects[3]!.write).toHaveBeenCalledOnce();
  for (const target of env.targetObjects) expect(target.destroy).not.toHaveBeenCalled();

  // A DPR change still changes star size and repel radius at the same backing size.
  renderer.resize({ width: 320, height: 160, dpr: 1 });
  flushResize();
  env.runFrame();
  expect(env.gpu.fns.target).toHaveBeenCalledTimes(11);
  expect(env.storageObjects[3]!.write).toHaveBeenCalledTimes(2);
  expect(env.computes[0]!.set.mock.calls.some(([value]) => value.params.pixelRatio === 1)).toBe(true);
  renderer.dispose();
});
