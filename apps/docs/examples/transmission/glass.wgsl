...
struct Glass {
  ...
  texel_angle: f32,
  /** Per-channel IOR spread used when dispersion is on. */
 ...
 */
fn transmitted_ray(entry: vec3f, incident: vec3f, normal: vec3f, eta: f32, exit_normal: vec3f, thickness: f32) -> vec4f {
  ...
  if (glass.refraction_mode < 0.5) { return vec4f(exit_position + inside * RAY_DISTANCE, 1.0); }
...
fn fs_main(in: VertexOut) -> @location(0) vec4f {
  ...
  let env_level = env_lod(glass.roughness * 0.5, dpdx(reflected), dpdy(reflected), glass.texel_angle);
  let scene_level = glass.roughness * max(glass.scene_levels - 1.0, 0.0);
  ...
  let thickness = select(glass.thickness, measured_thickness, glass.refraction_mode > 0.5);
  let exit_normal = normalize(select(-normal, back.xyz, glass.refraction_mode > 0.5));

  let reflection = sample_env(env_tex, env_samp, reflected, env_level, glass.env_size);

  // One index of refraction per channel when dispersion is on: red bends least, blue
  // most, so the three transmitted rays land on different pixels and the edges fringe.
  let spread = glass.dispersion * glass.dispersion_spread;
  let eta_r = 1.0 / (glass.ior - spread);
  let eta_g = 1.0 / glass.ior;
  let eta_b = 1.0 / (glass.ior + spread);

  let ray_g = transmitted_ray(in.world_position, incident, normal, eta_g, exit_normal, thickness);
  var transmitted = sample_transmission(ray_g, scene_level, reflection);
  if (glass.dispersion > 0.5) {
    let ray_r = transmitted_ray(in.world_position, incident, normal, eta_r, exit_normal, thickness);
    let ray_b = transmitted_ray(in.world_position, incident, normal, eta_b, exit_normal, thickness);
    transmitted = vec3f(
      sample_transmission(ray_r, scene_level, reflection).r,
      transmitted.g,
      sample_transmission(ray_b, scene_level, reflection).b,
    );
  }
