...
// radiance; alpha is deliberately independent from brightness so even a grey dot remains
// a solid occluder while the centre-out loading wave changes its emission.
...
fn dot_centre(row: i32, column: i32) -> vec2f {
...
fn centre_out_strength(centre: vec2f) -> f32 {
  ...
  // A matching smootherstep leaves a quiet grey beat on both sides of the loop.
...
fn edge_then_centre_strength(row: i32, column: i32) -> f32 {
...
fn animation_strength(row: i32, column: i32, centre: vec2f) -> f32 {
  if (agent.animation_mode == 1u) { return edge_orbit_strength(row, column); }
  if (agent.animation_mode == 2u) { return edge_then_centre_strength(row, column); }
  return centre_out_strength(centre);
}
...
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  for (var row = 0; row < 4; row = row + 1) {
    for (var column = 0; column <= row; column = column + 1) {
      let centre = dot_centre(row, column);
      let signed_distance = distance(pixel, centre) - agent.radius;
      let mask = 1.0 - smoothstep(-agent.edge_softness, agent.edge_softness, signed_distance);
      let emission = mix(agent.base_radiance, agent.peak_radiance, animation_strength(row, column, centre));
