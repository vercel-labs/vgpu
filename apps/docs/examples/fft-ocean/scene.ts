/** Shared FFT ocean graph and browser lifecycle; renderer.ts is public. */
...
export async function createGraph(gpu: Gpu, output: Output, label: string): Promise<OceanGraph> {
...
export function renderAt(gpu: Gpu, graph: OceanGraph, output: Target, time: number): void { setDynamics(graph, time); frame(gpu, (currentFrame) => renderGraph(currentFrame, graph, output)); }
export function renderGraph(currentFrame: Frame, g: OceanGraph, output: Output): void {
...
export function resizeOutputGraph(g: OceanGraph, output: Output): void {
...
export function destroyGraph(g: OceanGraph): void { for (const colorTarget of [g.noise, g.h0, g.spectrum, g.ping, g.pong, g.normalFoam, g.scene, g.bright, g.composite, ...g.levels.flatMap((x) => [x.horizontal, x.vertical])]) colorTarget.color.destroy(); }
