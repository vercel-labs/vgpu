...
@vertex fn vs_main(@location(0) position: vec3f, @location(1) normal: vec3f) -> VertexOut {
  ...
  let spin = scene.time * 0.28;
  let world = rotate_x(rotate_y(local, spin), 0.62);
  var out: VertexOut;
  out.position = scene.view_projection * vec4f(world, 1.0);
  out.local = local;
  out.normal = rotate_x(rotate_y(normal, spin), 0.62);
  return out;
}
