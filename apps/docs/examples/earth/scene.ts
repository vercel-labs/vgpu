/** Shared Earth scene and browser lifecycle; the public entry is renderer.ts. */
...
import { EARTH_TUNING, bloomSize, cameraBasis, normalizeSize, orbitPosition, sunDirection, type OrbitState } from './planet';
...
export function createMaps(gpu: Gpu, label: string): Maps {
...
export function createScene(gpu: Gpu, maps: Maps, label: string): Scene {
...
export function createTargets(gpu: Gpu, size: readonly [number, number], label: string): Targets {
...
export function setStaticBindings(scene: Scene, maps: Maps, targets: Targets): void {
...
export async function prewarm(scene: Scene, targets: Targets, output: Output): Promise<void> {
  await Promise.all([
...
export function render(currentFrame: Frame, scene: Scene, targets: Targets, output: Output): void {
