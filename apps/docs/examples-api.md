...
**Serving model: latest-only, straight from the deployment.** The artifact tree is generated by the docs `prebuild` step and ships inside the deployment bundle, so the docs deploy *is* the publication step. The tree is build output and is ignored by Git. There is no external object store, no credential, and no manual workflow on the default path: adding or editing an example is a normal commit, and the API serves the new revision as soon as that commit is deployed.
...
2. Commit the source change and refreshed source export. Do not commit `apps/docs/generated/examples-api`; it is ignored build output.
3. Merge. The docs `prebuild` step generates and publishes the new artifact tree; no workflow run and no library release are required.
