...
|---|---|---:|---|---|
| opts.shader | `string \| ShaderSource` | ✔ | — | WGSL string or loader-produced `ShaderSource`. Must contain compatible vertex/fragment entry points. When the fragment stage declares several entries and `entry.fragment` is omitted, a declared `fs_main` is preferred; otherwise the first fragment is used. Vertex selection remains first-in-stage when omitted. |
...
| opts.entry | `{ vertex?: string; fragment?: string }` | ✖ | `fs_main` for fragment when declared; otherwise first-in-stage | Selects which `@vertex`/`@fragment` functions to compile when one WGSL module declares several — variants of one technique sharing helpers, such as depth-only and shaded passes from the same source. Names must exist in the shader with the matching stage. An omitted fragment prefers a declared `fs_main`; other omissions use the first entry point of that stage. |
