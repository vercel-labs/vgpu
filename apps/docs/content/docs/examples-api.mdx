...
The examples API is the machine-readable source behind the vgpu examples workflow. Most users and coding agents should prefer `npx vgpu examples`, which performs compatibility checks, integrity verification, and safe filesystem writes.
...
```bash
npx vgpu examples
npx vgpu examples list
npx vgpu examples get gradient
```
