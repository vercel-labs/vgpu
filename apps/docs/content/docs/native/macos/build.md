...
The runtime-size follow-up validates multiple runtime storage buffers, sparse physical slots, stage-local packing, rebinding larger effective ranges, and equivalent immediate-data and legacy UBO results on the current Apple-silicon Metal runtime. For `buffer(i)`, word `i` is the effective bound range in bytes, and the region extends through the highest projected runtime-sized storage slot. C1 remains open for the exact-static materializer-to-worker integration, direct-target macOS 14 arm64 and x86_64 compiler builds, offline `metal` and `metallib`, the full shader corpus through that exact wrapper, authored spans beyond the current module-only attribution, artifact determinism, and pixel/buffer parity. The reproducible fixtures live in `experiments/native-metal-spikes/c1-override-defaults`, `experiments/native-metal-spikes/c1-runtime-buffer-sizes`, and `experiments/native-metal-spikes/c1-vertex-buffer-slots`.
