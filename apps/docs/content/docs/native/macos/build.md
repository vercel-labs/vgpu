...
C1 remains open for offline `metal` and `metallib`, a discriminated shader-interface projection, the full shader corpus through the exact direct worker, authored spans beyond the current module-only attribution, the end-to-end artifact, and pixel/buffer parity. The reproducible fixtures live in `experiments/native-metal-spikes/c1-override-defaults`, `experiments/native-metal-spikes/c1-override-worker-integration`, `experiments/native-metal-spikes/c1-runtime-buffer-sizes`, `experiments/native-metal-spikes/c1-vertex-buffer-slots`, `experiments/native-metal-spikes/c1-compiler-protocol`, and `experiments/native-metal-spikes/c1-tint-direct-build`.
