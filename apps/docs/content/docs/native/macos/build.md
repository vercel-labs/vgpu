...
The last locked direct-build revision compiled the C++ worker against Dawn/Tint commit `8f25b9c7064ae89802c8db4e7daab9d1fd3e77ca`, declaring only `tint_api` as its link root. Two clean Release builds for each of arm64 and x86_64 were byte-identical and combined into the same universal executable. The arm64 direct workers ran natively and the x86_64 direct workers ran under Rosetta; every variant produced responses byte-identical to the arm64-native monolithic reference. Their linked archive closure contained 44 Tint archives, nine Abseil archives, and `dawn_shared_utils`; the worker and JsonCpp translation units were linked directly. Those executables targeted macOS 14 and linked no WebGPU implementation, runtime backend, or framework. This proof predates the semantic-interface handshake and is intentionally stale until its source lock and hashes are rebaselined against the current worker; it is not current distribution evidence.
...
C1 remains open for offline `metal` and `metallib`, rebaselining the direct-source proof, the full shader corpus through that exact direct worker, authored spans beyond the current module-only attribution, the broader semantic-extractor-to-artifact integration, the end-to-end artifact, and pixel/buffer parity. The reproducible fixtures live in `experiments/native-metal-spikes/c1-override-defaults`, `experiments/native-metal-spikes/c1-override-worker-integration`, `experiments/native-metal-spikes/c1-runtime-buffer-sizes`, `experiments/native-metal-spikes/c1-vertex-buffer-slots`, `experiments/native-metal-spikes/c1-compiler-protocol`, `experiments/native-metal-spikes/c1-tint-direct-build`, and `experiments/native-metal-spikes/c1-shader-io-projection`.
