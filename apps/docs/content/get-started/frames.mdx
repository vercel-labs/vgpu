...
A frame is an optimization, not a requirement. Back-to-back one-shot draws also work — same passes, same order, just one submit each — and a frame with a single pass buys you nothing over `pulseEffect.draw({ target: canvasTarget })`. Reach for `gpu.frame()` when batching many passes into one submit starts to matter: heavy frames, or a render loop.
