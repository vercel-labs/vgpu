...
> Good to know: draws inside a bundle can use different shaders and pipelines. What a bundle freezes is the target's shape — color formats, depth format, sample count, size — not a material.
...
## Resizes invalidate a bundle

A bundle snapshots the target's size and formats when recorded. Replaying after a resize throws `VGPU-R3-BUNDLE-STALE` — re-record instead:
...
canvasTarget.onResize(() => { scene = recordScene(); }); // stale bundles throw on replay
