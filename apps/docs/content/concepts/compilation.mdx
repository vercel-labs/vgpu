...
[Bundles](/concepts/render-bundles) freeze the same signature tuple. Record with a signature after pre-warming every draw — `gpu.bundle(signature, ...)` then finds all pipelines in cache and records without any synchronous pipeline creation. If a bundle replays against a target whose signature doesn't match the recording, the error prints both keys, which is how you catch the `bgra8unorm` vs `rgba8unorm` surprise.
