...
Record what doesn't change, `set()` what does: the bundle references your buffers, so uniform updates flow through on every replay. Pre-warm the draws it records as described in [Compilation](/concepts/compilation) so recording never waits on pipeline creation.
