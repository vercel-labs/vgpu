...
struct Cascade {
  /** Atlas dimensions in texels: direction_base*scene, padded to probe spacing. */
  atlas_size: vec2f,
  ...
  has_upper: f32,
  /** Side of cascade 0's direction block: 2, 3, and 4 give 4, 9, and 16 rays. */
  direction_base: f32,
  _pad1: f32,
  _pad2: f32,
};
...
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  let block = rc_block_size(rc.cascade, rc.direction_base);
  let rays = rc_ray_count(rc.cascade, rc.direction_base);
  ...
  if (rc.has_upper > 0.5) {
    ...
    // centers into a lattice the bilinear weights can interpolate between.
