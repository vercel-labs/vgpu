/** Shared transmission scene and browser lifecycle; renderer.ts is public. */
...
export async function createScene(gpu: Gpu, output: Output, label: string): Promise<Scene> {
 ...
 */
export function renderScene(gpu: Gpu, scene: Scene, output: Output, view: CameraView, controls: TransmissionControls): void {
  const { targets } = scene;
