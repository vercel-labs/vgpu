...
import { checkImpact, parseChangeset, parseImpact, parseNotes, renderMigration, stableVersion } from "./lib/migrations.mjs";
import { checkMigrations, syncMigration } from "./migrations.mjs";
...
describe("release collection", () => {
  it("upserts RC fragments by ID and retains them after stable consumption", () => {
    ...
    const first = readFileSync(join(root, "docs/migrations/0.5.0.docs.md"), "utf8");
  ...
  it("renders a no-migration release and validates exact RC/stable versions", () => {
    expect(renderMigration({ version: "0.5.0", fragments: [parseChangeset(source(), "test")] })).toContain("No consumer migration");
  ...
  it("integrates with real Changesets across RC preparation and stable consumption", () => {
    ...
    run("version");
    syncMigration(root, "0.5.0", { "texture-resize": changeset });
    expect(() => checkMigrations(root, { release: true })).not.toThrow();
    expect(readFileSync(join(root, "docs/migrations/0.5.0.docs.md"), "utf8")).toContain("Create and rebind");
  }, 20_000);
