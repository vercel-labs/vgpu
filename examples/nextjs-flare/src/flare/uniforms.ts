...
import { lightPulse } from "./animation.ts";
import { gaussianBlurKernel, MAX_BLUR_TAPS } from "./blur-kernel.ts";
import {
  hexToRgb,
  type FlarePlacement,
  type FlareSettings,
...
function lightAttenuation(
  ...
  const localX =
    (light[0] - placement.logoCenter[0]) * placement.canvasToLogo[0];
  const localY =
    (light[1] - placement.logoCenter[1]) * placement.canvasToLogo[1];
...
export function setFrameUniforms(
  ...
  const blurTexel: readonly [number, number] = [
    1 / blurSize[0],
    1 / blurSize[1],
  ];
  effects.logo.set({
  ...
  effects.composite.set({
    params: {
      ...
      logoCenter: placement.logoCenter,
      flareColor,
      rimIntensity: intensity.rim * attenuation,
      extension: settings.extension,
      beamIntensity: intensity.beam * attenuation,
      ...
      // The shader is kept byte-identical to the marketing source; this
      // example ships the dark theme only.
      lightTheme: 0,
