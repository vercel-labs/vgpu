...
Tint may lower `arrayLength()` or robust runtime-array access through storage-buffer-size words. When the selected stage needs them, its Metal projection records a region inside the shared `immediate-data` payload. The physical slot and `immediate-data` role are explicit in the projection, never appear as a generated Swift binding, and cannot collide with user resources. A runtime-sized layout alone does not require a region; Tint may generate code that reads only its fixed prefix.
...
The artifact records the versioned packing model and stage-local region offset, but not concrete ranges, words, padding, or upload strategy. The runtime derives those values from the resource generations and ranges captured for each encoded command.
