...
The C1 compiler-protocol fixture now combines the virtual resolver, direct Tint API, and vgpu-owned slot map without calling Tint's automatic allocator. Its per-entry translation response is deliberately narrow: MSL, the selected entry point, the unchanged validated external map, effective internal bindings and size regions, the minimal Metal interface, and resolved compute workgroup dimensions. Fifteen positive and twenty-two negative native canaries cover stage-local resources, sparse WGSL groups, typed overrides, deterministic virtual-source diagnostics, multiple Metal resource namespaces, runtime storage sizes, complete shader interfaces, and fail-closed mismatches. The compiler validates requested remaps independently against Tint reflection and core IR before and after lowering instead of treating the request as its own evidence.

The same fixture preserves Tint ranges only in the resolved virtual WGSL and adds an authored module identity when the complete range lies within one proven module segment. It does not fabricate authored line or column positions. A separate override-materialization fixture has closed evaluated typed defaults and partial configuration through Tint's lowered IR, and its connected follow-up feeds the exact static set to the worker with source-hash and set equality checks. Assembling the multi-entry semantic union remains the next integration gate. The fixture's strict typed, finite scalar boundary does not claim complete WebGPU input-conversion parity. The sampled-texture resource binding-array canary now passes the isolated Tint writer and offline Apple compiler, but remains translator evidence only; resource binding arrays stay outside alpha because semantic v1 cannot carry their cardinality.

The C++ worker owns the final one-request stdin/EOF JSON codec. Its transport gate covers fragmented UTF-8, EOF framing, backpressure, cancellation, timeout, complexity limits, structured compiler failures, and untrustworthy process exits.
...
The runtime-size follow-up validates multiple runtime storage buffers, sparse physical slots, stage-local packing, rebinding larger effective ranges, and equivalent immediate-data and legacy UBO results on the current Apple-silicon Metal runtime. For `buffer(i)`, word `i` is the effective bound range in bytes, and the region extends through the highest projected runtime-sized storage slot. Its generated MSL also compiles and links offline for the macOS 14 target. The direct-target worker builds reproducibly for arm64, x86_64, and universal binaries, and the exact-static materializer handshake passes. C1 remains open for the semantic graph-to-request bridge, the full shader corpus through that exact worker and Apple's offline compiler, authored spans beyond current module-only attribution, a connected artifact, and pixel/buffer parity.
