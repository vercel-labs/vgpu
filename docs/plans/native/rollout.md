...
`immediate-data` slot. The fixture verifies that the storage-size model, regions, and internal slot
...
creation. A future-model descriptor proves that model support is checked only when the selected
program stage has a region. The fixture's sibling packages use relative paths only for isolation;
