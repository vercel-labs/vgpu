...
[Compute-to-draw indirect](./render/indirect.md). One nine-`u32` storage allocation is created with
`additionalUsage: [.indirect]`; compute changes the packet at bytes `16..<32` from its stale red
payload to the expected green payload, then a render frame consumes that exact bounded slice
...
Usage, alignment, and bounded-range negatives must fail as `VGPU-INDIRECT-INVALID`, while a
