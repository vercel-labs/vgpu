...
Carry that fixture into the production runtime, then run the direct DC1 gate specified in
...
the stale red triangle to the expected green triangle, then a render frame consumes that exact bounded slice
through a direct indirect draw. The zero-count packet at byte zero makes an ignored slice observable
as blue. The exact accepted
trace contains two submissions, with no packet readback, CPU wait, or intermediate settlement.
Usage, absolute alignment, and bounded-range negatives must fail as `VGPU-INDIRECT-INVALID`, while a
cross-context buffer preserves `VGPU-NATIVE-CONTEXT-MISMATCH`; all fail synchronously before work
registration or `onError`. DC1 is proposed and has not passed; there is no separate DC1a gate.
