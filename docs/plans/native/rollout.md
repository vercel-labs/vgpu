...
internal slots and size regions, and resolved compute workgroup dimensions. Ten positive and
sixteen negative native canaries are byte-deterministic across repeated runs.
...
spans beyond current module-only provenance, a discriminated
vertex-input/inter-stage/fragment-output projection, the full shader corpus through the exact
direct worker, deterministic connected artifact output, and pixel/buffer parity pass.
...
`experiments/native-metal-spikes/c1-runtime-buffer-sizes`, and
`experiments/native-metal-spikes/c1-tint-direct-build`.
