...
This guide targets `0.5.0-rc.1`. Choose the path matching your installed version.

## From the previous stable release

For projects on 0.4.x (the preceding stable release is 0.4.1), follow the applicable steps below.
Steps 1–5 affect texture creation, resizing, bindings and readback. Step 6 affects Node/Linux
applications, including systems with X11 or Wayland; other platforms do not need that backend change.
Steps 7–8 affect custom reflection and JavaScript-owned shader values, including WebGPU users
who never install native tooling. Step 9 is optional for native generation. Valid existing values
need no shape change. Adding the versioned CLI/MCP guides needs no application adaptation or
changes to existing CLI commands; discover them with `vgpu docs ls /migrations`.
...
install matching, exact RC versions as development dependencies:

```sh
npm install --save-dev --save-exact vgpu@0.5.0-rc.1 @vgpu/native@0.5.0-rc.1
```

Replace the empty `@vgpu/native@0.0.1` name reservation with this RC; it has no API or outputs
...
These changes are new in `0.5.0-rc.1`, including for browser/WebGPU users. Apply step 9 only if
...
The new CLI/MCP guides need no application or command changes. RC0 remains immutable;
bundled guides arrive in rc.1. No earlier RC exists in this cycle, and no texture/Vulkan reversal
needs undoing. The independent native bootstrap follows step 9, not the RC0 path.
