...
Choose the path matching your installed version. These instructions describe the final texture
contract and Linux backend behavior, not a sequence of changes to apply and undo. Each installed
package contains the guide as it existed at publication.
...
has adopted that RC, do not repeat steps 1–6: there is no additional API or backend migration in
the changes currently covered by this guide. If you installed the RC but have not adapted your
application yet, use the applicable steps from the stable-origin path.

Adding versioned guides to the CLI/MCP documentation requires no application changes or changes
to existing CLI commands. The already-published rc.0 package remains unchanged; the new bundled
guides become available in a subsequent release. No earlier published RC exists in this cycle.
