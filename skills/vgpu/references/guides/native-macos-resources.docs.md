...
The reflected runtime-sized layout records its alignment and minimum fixed prefix; its trailing array records the element layout and stride. It deliberately has no allocation-specific element count or final byte size. Those values belong to each storage resource and bound buffer range.

For a typed allocation, `count` produces a checked byte length from the fixed prefix plus `count × stride`. Writes preserve that declared extent. At binding time, the runtime validates the actual buffer offset and range against the fixed prefix and stride instead of changing the canonical layout to make the array appear statically sized. Raw buffers provide the equivalent extent through their explicit byte range.

Tint may lower robust runtime-array access through a storage-buffer-size table. The Metal runtime fills that table from the concrete ranges captured for the encoded command and binds it through an internal buffer slot reserved by vgpu. The slot and its role are explicit in the Metal projection, never appear as a generated Swift binding, and cannot collide with user resources.
